...
function CartPage() {
  ...
  if(isOrdered){
    return <OrderComplete>주문 완료</OrderComplete>
  }
    ...
    <Section>
      ...
      {cartItems.length === 0 ? (
        ...
        <>
          <List>
            {cartItems.map((item) => (
              <Item key={item.id}>
                ...
                <QuantityControl>
                  <QtyButton onClick={() => updateQuantity(item.id, -1)}>-</QtyButton>
          ...
          {/*총 금액 표시 */}
          <Total>총 결제 금액: {totalPrice.toLocaleString()}원</Total>
          {/*결제하기 버튼*/}
          <CheckoutButton onClick={handelCheckout}>결제하기</CheckoutButton>
        </>
      )}
    </Section>
  );
