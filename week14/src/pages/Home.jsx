...
import { getCookie, setCookie, removeCookie } from "../auth/cookieStorage";
...
const Home = () => {
  ...
  useEffect(() => {
    //이미 로그인 되어있으면 마이페이지로 이동
    const token=getAccessToken();
    if(token){
      navigate("/mypage",{replace:true});
      return;
    }
    const savedId = getCookie(SAVED_ID_COOKIE);
    if(savedId){
      onChangeId({target:{value:savedId}});
      setRememberId(true);
    }

  },[]);

  const onClick = async () => { //로그인버튼을 눌렀을 때 일어날 행동
    setErrorMessage("");
    try {
